...
router.post('/users', async (req, res) => {
  ...
  try {
    await addUser.save()
    sendWelcome(addUser.email, addUser.name)
...
const upload = multer({
  // dest:'avatars',  // remove that attribute to make multer upload file save string file name on database instead file local folder
  limits: {
    fileSize:1000000 
  },
  fileFilter(req, file, cb){
    if(!file.originalname.match(/\.(jpg|png|jpeg)$/gm)){
